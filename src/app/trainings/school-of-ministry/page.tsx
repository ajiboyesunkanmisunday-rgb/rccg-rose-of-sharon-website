...
export default function SchoolOfMinistryPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    try {
      await post("/school-of-ministries", {
        firstName: form.firstName, middleName: form.middleName || undefined,
        lastName: form.lastName, countryCode: form.countryCode, phoneNumber: form.phoneNumber,
        ...
        goneThroughNewConverts: form.goneThroughNewConverts === "true",
        ...
        reasonsForApplying: form.reasonsForApplying ? [form.reasonsForApplying] : [],
    ...
    <main>
      <section className="relative min-h-[400px] md:h-[500px] w-full overflow-hidden flex items-center justify-center">
        ...
        <div className="relative z-10 w-full max-w-[860px] px-6 mt-16 md:mt-[48px] flex flex-col gap-[16px] items-center text-center">
          ...
          <h1 className="text-[#FFFDFD] text-[36px] md:text-[56px] lg:text-[72px] font-medium leading-tight w-full" style={vs}>School of Ministry</h1>
          <p className="text-[#FFFDFD] text-[16px] md:text-[20px] font-normal leading-[1.5] md:leading-[32px]" style={vs}>Comprehensive training for those called to ministerial and pastoral work.</p>
        </div>
      </section>
      ...
      <section className="bg-[#F3F4F6] px-4 sm:px-[40px] lg:px-[80px] xl:px-[120px] py-[60px] md:py-[84px] flex flex-col gap-[32px] items-center w-full">
        <div className="flex items-center gap-[8px]">
          ...
          <span className="text-[#111827] text-[16px]" style={vs}>School of Ministry</span>
        </div>
        <h2 className="text-[#000080] text-[28px] md:text-[40px] lg:text-[48px] font-bold text-center" style={vs}>SOM Application Form</h2>
        ...
        {status === "success" ? (
          <div className="flex flex-col gap-[24px] items-center max-w-[600px] text-center">
            ...
            <p className="text-[#6B7280] text-[16px]" style={vs}>Your School of Ministry application has been received. Our team will be in touch.</p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-[16px] w-full max-w-[900px]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-[16px] w-full">
              ...
              <FormField label="Place of Work"><div className={wrapCls}><input type="text" value={form.placeOfWork} onChange={set("placeOfWork")} className={inputCls} /></div></FormField>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-[16px] w-full">
            ...
            <FormField label="Have You Gone Through New Convert Class?" required>
              <div className={wrapCls}><select required value={form.goneThroughNewConverts} onChange={set("goneThroughNewConverts")} className={selectCls}><option value="false">No</option><option value="true">Yes</option></select></div>
            </FormField>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-[16px] w-full">
            ...
            <FormField label="Reason for Applying" required>
              <div className="bg-[#FFFDFD] border border-[#A3A1AF] rounded-[10px] w-full min-h-[120px]"><textarea required value={form.reasonsForApplying} onChange={set("reasonsForApplying")} className="w-full h-full min-h-[120px] px-3 py-2 bg-transparent text-[#100E1A] text-sm focus:outline-none rounded-[10px] resize-none" /></div>
            </FormField>
