  ...
  return (
    <main>
      ...
      <section className="relative h-[500px] w-full overflow-hidden flex items-center justify-center">
        ...
        <div className="absolute left-1/2 top-[calc(50%+48px)] -translate-x-1/2 -translate-y-1/2 flex flex-col gap-[20px] items-center w-[1148px] z-10 text-center">
          <p className="text-[#B5B5F3] text-[15px] font-normal uppercase tracking-[0.2em]" style={vs}>Church Calendar</p>
          <h1 className="text-[#FFFDFD] text-[72px] font-medium leading-tight" style={vs}>Events &amp; Services</h1>
          <p className="text-[#FFFDFD]/80 text-[18px] font-normal leading-[1.7] max-w-[600px]" style={vs}>
            Stay connected with everything happening at RCCG Rose of Sharon.
          </p>
          {/* Legend */}
          <div className="flex gap-[16px] flex-wrap justify-center mt-[4px]">
            {Object.entries(CAT).map(([k, c]) => (
              <div key={k} className="flex items-center gap-[6px]">
                ...
                <span className="text-[#FFFDFD]/70 text-[12px] font-normal" style={vs}>{c.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── Calendar + Sidebar ── */}
      <section className="bg-[#EEF0F7] px-[64px] py-[64px] flex gap-[40px] items-start w-full min-h-screen">

        {/* LEFT — calendar card */}
        <div className="flex-1 min-w-0 rounded-[24px] overflow-hidden shadow-[0_8px_48px_rgba(0,0,128,0.10)] bg-white">

          {/* Month nav */}
          <div className="bg-[#000080] flex items-center justify-between px-[32px] py-[28px]">
            <button onClick={prev} aria-label="Previous month"
              className="size-[44px] rounded-full bg-white/10 hover:bg-white/25 flex items-center justify-center transition-colors">
              <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
            </button>
            <div className="flex flex-col items-center gap-[2px]">
              <h2 className="text-[#FFFDFD] text-[32px] font-bold leading-none" style={vs}>{MONTHS[month]}</h2>
              <span className="text-[#B5B5F3] text-[16px] font-normal" style={vs}>{year}</span>
            </div>
          ...
          <div className="grid grid-cols-7 bg-[#000080]/5 border-b border-[#000080]/10">
            {DAYS.map(d => (
              <div key={d} className="py-[10px] text-center text-[#000080] text-[12px] font-bold uppercase tracking-widest" style={vs}>{d}</div>
            ))}
          </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-7">
              {Array.from({ length: firstDow }).map((_, i) => (
                <div key={`pad-${i}`} className="h-[108px] border-b border-r border-gray-100 bg-gray-50/50" />
              ))}
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      "h-[108px] p-[10px] flex flex-col items-start text-left transition-all border-b border-r border-gray-100 group relative overflow-hidden",
                  ...
                  >
                    {/* Day number */}
                    <span className={[
                      "text-[13px] font-bold w-[28px] h-[28px] flex items-center justify-center rounded-full flex-shrink-0 z-10",
                    ...
                    {/* Event pills */}
                    <div className="flex flex-col gap-[2px] w-full mt-[4px] overflow-hidden">
          ...
          {selected && (
            <div className="border-t-2 border-[#000080]/10 bg-[#F8F8FF]">
              <div className="px-[32px] pt-[24px] pb-[8px] flex items-center justify-between">
                <h3 className="text-[#100E1A] text-[16px] font-bold" style={vs}>{formatFull(selected)}</h3>
              ...
              {selectedEvts.length === 0 ? (
                <p className="px-[32px] pb-[24px] text-[#A3A1AF] text-[14px]" style={vs}>No events scheduled for this day.</p>
              ) : (
                <div className="px-[32px] pb-[24px] flex flex-col gap-[12px]">
                  {selectedEvts.map((ev, i) => {
                    const cfg = CAT[ev.eventCategory ?? ""] ?? CAT.SERVICE;
                    return (
                      <div key={i} className="bg-white rounded-[16px] p-[20px] flex gap-[16px] items-start shadow-[0_2px_12px_rgba(0,0,128,0.07)]">
                        <div className="w-[4px] self-stretch rounded-full flex-shrink-0" style={{ background: cfg.bg }} />
                        <div className="flex flex-col gap-[6px] flex-1 min-w-0">
                          <div className="flex items-center gap-[8px] flex-wrap">
                            <span className="text-[#100E1A] text-[17px] font-bold" style={vs}>{ev.title}</span>
        ...
        {/* RIGHT — sidebar */}
        <div className="w-[340px] flex-shrink-0 flex flex-col gap-[24px]">
          ...
          <div className="flex flex-col gap-[4px]">
            ...
            <h2 className="text-[#100E1A] text-[24px] font-bold" style={vs}>Upcoming Events</h2>
          </div>
      ...
      <section className="bg-[#100E1A] px-[120px] py-[64px] flex flex-col gap-[16px] items-center w-full text-center">
        <p className="text-[#FFFDFD] text-[24px] font-normal italic leading-[1.6] max-w-[700px]" style={{ fontFamily: "'Playfair Display', serif" }}>
          &ldquo;Not neglecting to meet together, as is the habit of some, but encouraging one another.&rdquo;
        </p>
        <p className="text-[#B5B5F3] text-[15px] font-semibold uppercase tracking-widest" style={vs}>Hebrews 10:25</p>
      </section>
